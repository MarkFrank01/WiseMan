...
/**
 * ================================================ 作    者：廖子尧 版    本：1.0 创建日期：2016/1/7 描    述：
 * Matrix 的9个值分别为  缩放  平移  倾斜 MSCALE_X	 MSKEW_X	MTRANS_X MSKEW_Y	 MSCALE_Y	MTRANS_Y MPERSP_0
 * MPERSP_1	MPERSP_2 修订历史： ================================================
 */
...
public class CropImageView extends AppCompatImageView {
	...
	private int mBorderColor = 0x00000000; //焦点框的边框颜色
	private int mBorderWidth = 0;         //焦点边框的宽度（画笔宽度）
	...
	public CropImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		mBorderWidth = a
			.getDimensionPixelSize(R.styleable.CropImageView_cropBorderWidth, mBorderWidth);
		mFocusWidth = a
			.getDimensionPixelSize(R.styleable.CropImageView_cropFocusWidth, mFocusWidth);
		mFocusHeight = a
			.getDimensionPixelSize(R.styleable.CropImageView_cropFocusHeight, mFocusHeight);
	...
	private void initImage() {
		...
		if (mStyle == Style.CIRCLE) {
			int focusSize = getWidth() - ScreenUtils.dip2px(20) * 2;
//            int focusSize = Math.min(mFocusWidth, mFocusHeight);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (Style.RECTANGLE == mStyle) {
			...
			float radius = Math
				.min((mFocusRect.right - mFocusRect.left) / 2,
		...
		switch (event.getAction() & MotionEvent.ACTION_MASK) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mode == DRAG) {
					...
					float newDist = spacing(event.getX(0), event.getY(0), event.getX(1),
						event.getY(1));
					if (newDist > 10f) {
						matrix.set(savedMatrix);
					...
					if (b > 10) {
						...
						matrix.set(savedMatrix);
						matrix
							.postRotate((float) (rotation * 180 / Math.PI), midPoint.x, midPoint.y);
						setImageMatrix(matrix);
					}
				}
				break;
	...
	private void fixScale() {
		...
		float minScale = getScale(mRotatedImageWidth, mRotatedImageHeight, mFocusWidth,
			mFocusHeight,
			true);
	...
	public Bitmap rotate(Bitmap bitmap, int degrees) {
		if (degrees != 0 && bitmap != null) {
			...
			matrix
				.setRotate(degrees, (float) bitmap.getWidth() / 2, (float) bitmap.getHeight() / 2);
			try {
...
//                    bitmap.recycle();
			...
			if (expectWidth != width || exceptHeight != height) {
				...
				if (mStyle == CropImageView.Style.CIRCLE && !isSaveRectangle) {
					...
					Bitmap circleBitmap = Bitmap
						.createBitmap(length, length, Bitmap.Config.ARGB_8888);
	 ...
	 * 将图片保存在本地
	 */
	private void saveOutput(Bitmap croppedImage, Bitmap.CompressFormat outputFormat,
		File saveFile) {
		OutputStream outputStream = null;
		try {
			outputStream = getContext().getContentResolver()
				.openOutputStream(Uri.fromFile(saveFile));
