...
class ArticleAndSubjectAdapter(data: List<ArticleAndSubjectBean>?, private val mListener: SubjectOnClickListener) :
	...
	private fun initArticleView(helper: BaseViewHolder, bean: ArticleAndSubjectBean) {
		...
		val imageView = helper.getView<ImageView>(R.id.iv_item_article_icon)
		val imageUrl = ZhiZheUtils.getHDImageUrl(item.imageUrl)
		ImageLoader.load(mContext, imageUrl, R.drawable.default_card, imageView)

		helper.setText(R.id.tv_item_article_title, item.name)
		helper.setText(R.id.tv_item_article_category, item.categoryName)
		helper.setText(R.id.tv_item_article_label, item.labelName)
		helper.setText(R.id.tv_item_article_read, item.readNum.toString())
		helper.setText(R.id.tv_item_article_comment, item.commentNum.toString())

		imageView.transitionName = mContext.getString(R.string.card_img_transition_name)
		helper.getView<TextView>(R.id.tv_item_article_title).transitionName = mContext.getString(
				R.string.card_title_transition_name)
		helper.getView<TextView>(R.id.tv_item_article_category).transitionName = mContext.getString(
				R.string.card_category_transition_name)
		helper.getView<TextView>(R.id.tv_item_article_label).transitionName = mContext.getString(
